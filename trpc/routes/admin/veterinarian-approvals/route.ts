...
export const veterinarianApprovalsRouter = {
  ...
  approveApplication: protectedProcedure
    ...
    .mutation(
      async ({ input, ctx }: { input: { adminId: number; applicationId: string; adminNotes?: string }; ctx: any }) => {
        try {
    ...
    .query(async ({ input, ctx }: { input: { userId: string; limit?: number }; ctx: any }) => {
      try {
        ...
        const userNotifications = await db
          .select()
          .from(notifications)
          .where(and(eq(notifications.userId, userId), eq(notifications.type, "veterinarian_approval")))
        ...
        const formattedNotifications = userNotifications.map((notif) => ({
          ...
          status: notif.type === "veterinarian_approval" ? "approved" : "rejected",
