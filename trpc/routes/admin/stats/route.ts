...
import { db, users, pets, inquiries, consultations, vetStores, storeProducts, approvalRequests, petApprovalRequests, veterinarians } from '../../../../db';
...
export const getSystemStatsProcedure = publicProcedure
  ...
  .query(async ({ input }: { input: { adminId: number } }) => {
    try {
      ...
      const [pendingApprovalsCount] = await db.select({ count: count() }).from(approvalRequests).where(eq(approvalRequests.status, 'pending'));
      const [pendingPetApprovalsCount] = await db.select({ count: count() }).from(petApprovalRequests).where(eq(petApprovalRequests.status, 'pending'));
      const [pendingVetApprovalsCount] = await db.select({ count: count() }).from(veterinarians).where(eq(veterinarians.isVerified, false));
      
      // Mock field assignments count (replace with actual query when table is ready)
      const pendingFieldAssignments = 5;
...
export const getDetailedStatsProcedure = publicProcedure
  ...
  .query(async ({ input }: { input: { adminId: number; category: 'users' | 'pets' | 'inquiries' | 'consultations' | 'stores' } }) => {
    try {
  ...
  .query(async ({ input }: { input: { adminId: number } }) => {
    try {
      ...
      return {
        ...
        pendingVetApprovals: pendingVetApprovalsCount.count || 0,
        pendingFieldAssignments: pendingFieldAssignments,
        total: (pendingApprovalsCount.count || 0) + (pendingPetApprovalsCount.count || 0) + (pendingVetApprovalsCount.count || 0) + pendingFieldAssignments
      };
    } catch (error) {
