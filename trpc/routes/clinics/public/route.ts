  ...
  .query(async ({ input }) => {
    try {
      ...
      console.log("Database query result:", {
        ...
        activeClinics: activeClinics.map((c) => ({ id: c.id, name: c.name, isActive: c.isActive })),
      });

      // If no clinics found in database, return mock data for development
      console.log("Database clinics result:", {
        activeClinicsLength: activeClinics.length,
        activeClinics: activeClinics.map((c) => ({ id: c.id, name: c.name })),
      });
      ...
      return {
        ...
        clinics: activeClinics.map((clinic: any) => ({
          ...clinic,
          workingHours:
            typeof clinic.workingHours === "string" ? JSON.parse(clinic.workingHours) : clinic.workingHours ?? null,
